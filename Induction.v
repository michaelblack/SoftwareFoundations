...
Proof. intros b. intros c. destruct b. 
  Case "b = true". simpl. intros. rewrite H. reflexivity.
  Case "b = false". simpl. intros. destruct c.
...
Theorem mult_dist_l : forall n m o, n * (m + o) = n * m + n * o.
Proof. intros. induction n.
  simpl. reflexivity.
  simpl. rewrite  IHn. rewrite <- plus_assoc. 
    assert (o + (n * m + n * o) = (n * m + n * o) + o). rewrite plus_comm. reflexivity.
    assert (o + n * o = n * o + o). rewrite plus_comm. reflexivity.
    rewrite H. rewrite H0. rewrite plus_assoc. rewrite plus_assoc. rewrite plus_assoc. reflexivity.
Qed.
...
Proof. intros. induction m as [|m'].
  ...
  Case "S m'".
    simpl. assert (H : S m' = 1 + m'). simpl. reflexivity.
      rewrite H. rewrite mult_dist_l. rewrite mult_1_r. rewrite IHm'. reflexivity.
Qed.
...
Proof. intros. induction n.
  simpl. reflexivity. 
    assert (H : negb (evenb n) = evenb (S n)).
      rewrite IHn. rewrite negation_fn_applied_twice. reflexivity.
        reflexivity.
    assert (H1 : evenb n = evenb (S (S n))). simpl. reflexivity.
...
Theorem zero_nbeq_S : forall n:nat,
  beq_nat 0 (S n) = false.
Proof. Proof. intros. induction n.
  simpl. reflexivity.
  simpl.  reflexivity.
Qed.
...
Proof. intros.
  rewrite mult_comm.
   assert (H1 : n * p = p * n). rewrite mult_comm. reflexivity.
   assert (H2 : m * p = p * m). rewrite mult_comm. reflexivity.
 rewrite H1. rewrite H2. rewrite mult_dist_l. reflexivity.
Qed.
...
Proof. intros. induction p.
  ...
  rewrite plus_assoc. rewrite plus_assoc. replace (n + m) with (m + n). reflexivity.
  rewrite plus_comm. reflexivity.
Qed.
(** [] *)
...
(** Theorem: Addition is commutative.
      Forall n and m, n + m = m + n
    
    Proof by induction on m.
      Suppose m is 0. Both n + 0  = n by plus's right identity and 0 + n = n by definition of plus.
        Therefor we have n = n which is obviously true and the base step is true.
        
      Now assuming the inductive hypthesis (n + m = m + n) is true, we must show that n + S m = S m + n is true.
      By definition of plus, we have n + S m = S (m + n) and by the inductive hypthesis S (m + n) is S (n + m).
      We also know that S distributes over the right operand, therefore S (n + m) is n + S m.
      Therefore we have n + S m = n + S m which is obviously true.
...
(** Write an informal proof of the following theorem, using the
    ...
    Proof: We prove the above by induction on n.
      The 0 case is trivialy true by definition of beq_nat. 
      Additionally, the case of beq_nat (S n) (S n) reduces to the inductive hypthosesis by definition of beq_nat as well.
      Therefore we have both the base and inductive step and we have our proof.
[] *)
