...
Proof. intros. destruct p. simpl. reflexivity. Qed.
(** [] *)
...
Fixpoint filter (f : nat -> bool) (l : natlist) := 
  match l with
   | nil => nil
   | x :: xs => match f x with
                       | true =>  x :: filter f xs
...
Definition notbn (f : nat -> bool)  (n : nat) := negb (f n).

Definition oddmembers (l:natlist) : natlist := filter (notbn evenb) l.
...
Definition remove_all (v:nat) (s:bag) : bag := filter (notbn (beq_nat v)) s.
...
Theorem refl_nat : forall n, beq_nat n n = true.
  Proof. induction n.
    reflexivity.
    simpl. rewrite IHn. reflexivity.
  Qed.

Theorem add_remove_id : forall (v : nat) (l : bag), remove_one v (add v l) = l.
  Proof. intros. induction l, v.
    reflexivity.
    simpl. rewrite refl_nat. reflexivity.
    simpl. reflexivity.
    simpl. rewrite refl_nat. reflexivity.
 Qed.
(** [] *)
...
Theorem rev_snoc_is_cons_rev : forall (n : nat) (l : natlist),
...
Proof. induction l.
  reflexivity.
  simpl. rewrite rev_snoc_is_cons_rev. rewrite IHl. reflexivity.
Qed.
...
Theorem snoc_dist_r : forall (a : nat) (xs ys : natlist), snoc (xs ++ ys) a = xs ++ (snoc ys a).
...
Proof. intros. 
  ...
  simpl. rewrite app_nil_end. reflexivity.
  simpl. rewrite IHl1. simpl.  rewrite snoc_dist_r. reflexivity.
Qed.
...
Proof. intros. induction l.
  reflexivity.
  simpl. rewrite refl_nat. rewrite <- IHl. simpl. reflexivity.
Qed.
(** [] *)
...
Lemma singleton_app_is_cons : forall (b : nat) (bs : natlist), b :: bs = [b] ++ bs.
  Proof. intros. simpl. reflexivity. Qed.

Lemma snoc_cons_side : forall (n : nat) (xs ys : natlist), snoc xs n ++ ys = xs ++ cons n ys.
Proof. intros.  rewrite snoc_append. assert (H :  (n :: ys) = [n] ++ ys).
  rewrite singleton_app_is_cons. reflexivity.
...
Theorem count_over_sum_is_add : forall (n : nat) (xs ys : natlist), count n (sum xs ys) = (count n xs) + (count n ys).
Proof. intros. induction xs.
  simpl. reflexivity.
  simpl. destruct (beq_nat  n n0).
  simpl. rewrite <- IHxs. reflexivity.
...
Proof. intros.
  rewrite <- rev_involutive. symmetry. rewrite <- rev_involutive.
