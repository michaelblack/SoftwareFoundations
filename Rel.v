...
Theorem not_partial_total : ~ (partial_function total_relation).
Proof.
  unfold not. unfold partial_function. intro. 
  assert (0 = 1) as Nonsense.
  apply H with (x := 0). apply tot.
  apply tot.
  inversion Nonsense.
Qed.
(* FILL IN HERE *)
...
Theorem partial_empty : partial_function empty_relation.
  Proof. unfold partial_function. intros. inversion H. Qed.
...
Proof.
  ...
  induction o as [| o'].
    inversion Hmo.
    inversion Hmo. apply le_S. rewrite <- H0. apply Hnm.
...
Theorem le_S_n : forall n m,
  (S n <= S m) -> (n <= m).
Proof.
  intros. inversion H. reflexivity.  apply le_Sn_le. apply H1. Qed.
(** [] *)
...
(** Provide an informal proof of the following theorem:
    ...
    Proof: By induction over the natural n
    
    Case n = 0.
      Suppose n is 0. We must prove 1 <= 0 implies False.
       This is, by definition, a contradiction, 
       therefor we can prove this case via inversion on 1 <= 0.
       
    Case n = S n'.
      Suppose n is S n'. We must prove S (S n') <= S n'  -> False from the
      hypothesis S n' <= n'. -> False. By assuming S n' <= n', we can then say that
      proving S (S n') <= S n' suffices to prove the goal False. By definition of  le,
      this is the same as proving S n' <= n', which is our very assumption.
...
Theorem le_not_symmetric :
  ~ (symmetric le).
Proof.
  unfold not. unfold symmetric. intro H. assert (H0 : 0 <= 1). apply le_S. apply le_n. apply H in H0. inversion H0. Qed.
(** [] *)
...
Theorem le_antisymmetric :
  antisymmetric le.
Proof.
  unfold antisymmetric. intro a. intro b. intros. inversion H. reflexivity. rewrite <- H2 in H0. apply (le_trans (S m) a (m)) in H0.  apply le_Sn_n in H0. inversion H0.  apply H1. Qed.
(** [] *)
...
Theorem le_step : forall n m p,
  ...
  n <= p.
Proof. 
  intros. unfold "<" in H.  apply (le_trans (S n) m (S p)) in H. apply le_S_n in H. apply H. apply H0. Qed.
(** [] *)
...
Theorem rsc_trans :
  forall (X:Type) (R: relation X) (x y z : X),
      ...
      refl_step_closure R x z.
Proof.
  intros X R x y z Rxy Ryz. induction Rxy.  
      apply Ryz. 
      apply rsc_step with y. apply H. apply IHRxy. apply Ryz.
Qed.
(** [] *)
...
Proof.
  intros X R x y. split. 
      intro crt. induction crt. 
           apply rsc_R. apply H.
           apply rsc_refl. apply (rsc_trans X  R x y z)  in IHcrt1. 
              apply IHcrt1.
              apply IHcrt2. 
     intro rsc. induction rsc.
        apply rt_refl.
        apply (rt_step R x y) in H. apply (rt_trans R x y z)  in H .
            apply H.
            apply IHrsc.
Qed.
