...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
  Rewrite matches terms and writes equalities. Apply eliminates goals that match a hypothesis or theorem.
  situations with [rewrite H. reflexivity.] will often be doable with [apply H.]
*)
(** [] *)
...
Proof.
  ...
  destruct m.
    ...
    intros. 
        simpl in H. 
        rewrite <- plus_n_Sm in H. symmetry in H. rewrite <- plus_n_Sm in H. 
        inversion H.
        symmetry in H1. apply IHn' in H1.
        rewrite H1.
        reflexivity.
Qed. 
(** [] *)
...
Proof. 
  ...
  destruct m. simpl. intro contra. inversion contra.
                     intro H. simpl in H. apply IHn in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
(*
We will prove 
  forall m, beq_nat n m = true -> n = m
by induction on n
 int he case that n = 0, let us examine two possible cases for m.
  if m is 0, then it is obviously true because 0 = 0
  if m is S of some number, then then we have proven the absurd in the hypothetical and the conclusion is trivially true.
Once we have proven the 0 case, we must show that 
  forall m, beq_nat (S n) m = true -> (S n) = m
  again, we can examine two possible cases for m.
  If m is 0, then then we have proven the absurd in the hypothetical and the conclusion is trivially true again.
  If m is S m' then, after introduce they hypothetical, the definition of beq_nat gives us the hypothetical of the inductive hypothesis.
  from there we gain n = m' from which it is trivial to prove S n = S m'. *)
(** [] *)
...
Proof. intros.
  ...
  induction l. 
      simpl.  reflexivity.
      destruct n. intro H. simpl in H. inversion H.
                        simpl. intro H.  apply IHl. inversion H. reflexivity.
Qed. 
(** [] *)
...
(** Write an informal proof corresponding to your Coq proof
    of [index_after_last]:
     ...
     _Proof_:
     Let X be some type and l be some list of X. We will prove the theorem by induction on l.
     
     If l is []. Then, regardless of n, index n l is None. And thus, given the hypothesis, we must prove None = None. 
     This is trivially true under any hypothesis.
     
     In the case that l is x :: xs. We will introduce n as some nat and consider two cases for it.
        In the case of n is 0, by definition, the length of x :: xs is S of some nat, but we have declared it equal to 0 in the hypothesis 
        and have therefor introduced the absurd from which we can prove anything.
        
        in the case of n is S n', we have, by definition of length, the hypothesis of S (length l) = S n' and from it we must prove index n l = None.
        However, the inductive hypothesis says that to prove this, it is sufficient to prove length l = n. By the injectivity of of S, we can prove this 
        from out hypthesis.
...
Proof.
  ...
  induction l. simpl. destruct n. reflexivity.
                                                intro H. inversion H.
  simpl. destruct n. intro H. inversion H.
                              intro H. inversion H. apply IHl in H1. apply f_equal. rewrite H. rewrite H1. reflexivity.
Qed.
(** [] *)
...
Theorem self_implication : forall a, a -> a.
...
Proof.
  ...
  induction l1.
     ...
     simpl. destruct n. intros. inversion H.
                                 simpl. intros. inversion H. apply IHl1 in H1. rewrite H1. rewrite H. reflexivity.
Qed.
...
Lemma length_s_r : forall (X : Type)  (l1 : list X) (l2 : list X) (x : X), length (l1 ++ x :: l2) = S (length (l1 ++ l2)).
  Proof. intros X l1. induction l1. simpl. reflexivity.
...
Proof. intros.  generalize dependent n. 
  induction l. 
    ...
    simpl. destruct n. intros. inversion H. 
               intros. rewrite length_s_r. inversion H. apply IHl in H1. rewrite H. simpl. rewrite <- plus_n_Sm. rewrite H1. reflexivity.
Qed.
...
Theorem double_induction: forall (P : nat -> nat -> Prop), 
  ...
  forall m n, P m n.
Proof. intros p a b c d m.
         induction m.
          induction n. apply a. 
                              apply c. apply IHn.
          induction n. apply b. apply IHm.
                             apply d. apply IHm.
Qed.
(** [] *)
...
Proof. unfold override. intros. destruct (beq_nat k1 k2).
        reflexivity.
        reflexivity.
Qed.
(** [] *)
...
Proof. intros X Y l. induction l.
    ...
    intros l1 l2 H. destruct x. simpl in H. destruct (split l) as [xs ys].
...
Proof. intros. destruct b.
  destruct (f true) eqn:ft0. rewrite ft0. apply ft0.
  destruct (f false) eqn:ff0. apply ft0. apply ff0.
  destruct (f false) eqn: ff0. destruct (f true) eqn:ft0. apply ft0. 
  apply ff0. rewrite ff0. apply ff0.
Qed.
(** [] *)
...
Proof.
  ...
  destruct (beq_nat k1 k2) eqn:bkk.
      apply beq_nat_true in bkk. rewrite <- H. apply f_equal. apply bkk.
      reflexivity.
Qed.
...
(** Give an informal proof of this lemma that corresponds to your
   ...
   Proof: We will prove the above by induction on n,
   For the first part we must prove
      ∀m, beq_nat 0 m = beq_nat m 0,
      Here we must simply consider the two possibility for m :
            If it is 0, then both statements are true and true = true
            If it is S _ then both statements are false and false = false
  For the second part of the induction we must prove
    ∀m, beq_nat (S n') m = beq_nat m (S n') from the induction hypthesis ∀m. beq_nat n' m = beq_nat m n'
    Here we again must only consider two possiblilities for m:
          If it is 0, then both statements are false and false = false
         If it is S m' then by definition of beq_nat this is the same as beq_nat n' m' = beq_nat m' n'
              This is simply the induction hypthesis with m' substituted for the universal m.
              
  Having shown both parts of the induction we have completed our proof.
[]
 *)
...
Proof. 
  ...
  intros. destruct l2.
             simpl in H. inversion H.
             simpl in H. inversion H. apply IHl1 in H1. simpl. rewrite H1. reflexivity.
Qed.
...
Proof. 
  ...
  destruct (beq_nat k2 k1) eqn:eq. 
      intros. inversion H.
      intros. destruct (beq_nat k1 k3) eqn:eq1. 
          destruct (beq_nat k2 k3) eqn:eq2.
              apply beq_nat_true in eq2. rewrite <- eq2 in eq1. rewrite beq_nat_sym in eq1. rewrite eq1 in eq. inversion eq.
              reflexivity.
         destruct (beq_nat k2 k3) eqn:eq2.
         reflexivity.
         reflexivity.
...
Proof.
 ...
 generalize dependent test.
  induction l. 
    intros. inversion H.
    destruct lf. simpl. intros. destruct (test x) eqn:eq1. 
                                            inversion H. rewrite <- H1. apply eq1.
                                            apply IHl with (lf := []). apply H.
    simpl. intros. destruct (test x) eqn:eq1.
                          inversion H. rewrite <- H1. apply eq1.
...
Fixpoint forallb {X : Type} (p : X -> bool) (l : list X) : bool :=
  match l with
   | [] => true
   | x :: xs => if negb (p x) then false else forallb p xs
 end.
...
Example testLastEx1 : forallb oddb [1;3;5;7;9] = true.
Proof. reflexivity. Qed.

Example testLastEx2 : forallb negb [false;false] = true.
Proof. reflexivity. Qed.

Example testLastEx3 : forallb evenb [0;2;4;5] = false.
Proof. reflexivity. Qed.

Example testLastEx4 : forallb (beq_nat 5) [] = true.
Proof. reflexivity. Qed.

Example testLastEx5 : existsb (beq_nat 5) [0;2;3;6] = false.
Proof. reflexivity. Qed.

Example testLastEx6 : existsb (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.

Example testLastEx7 : existsb oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.

Example testLastEx8 : existsb evenb [] = false.
Proof. reflexivity. Qed.

Example testLastEx9 : existsb' (beq_nat 5) [0;2;3;6] = false.
Proof. reflexivity. Qed.

Example testLastEx10 : existsb' (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.

Example testLastEx11 : existsb' oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.

Example testLastEx12 : existsb' evenb [] = false.
Proof. reflexivity. Qed.

Theorem existsb_existsb' : forall (X : Type) (l : list X) (p : X -> bool), existsb p l = existsb' p l.
  Proof.
  intros X l.
  induction l.
  unfold existsb'. simpl. reflexivity.
  simpl. intro p. destruct (p x) eqn:P.
                        unfold existsb'. simpl. rewrite P. simpl. reflexivity.
                                                 rewrite IHl. unfold existsb'. simpl. rewrite P. simpl. reflexivity.
Qed.
