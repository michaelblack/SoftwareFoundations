...
Eval compute in test_ceval empty_state loop.
...
Eval compute in test_ceval (update empty_state X 63) even_z.
(* ==> Some (1, 0, 1) *)
Eval compute in test_ceval (update empty_state X 44) even_z.
(* ==> Some (0, 0, 0) *)
Eval compute in test_ceval (update empty_state X 2) even_z.
(* ==> Some (0, 0, 0) *)
Eval compute in test_ceval (update empty_state X 1) even_z.
(* ==> Some (1, 0, 1) *)
(** [] *)
...
(** Write an informal proof of [ceval_step__ceval], following the
 ...
 _Theorem_: forall (c : com) (st st' : state),
      (exists (i : nat), ceval_step st c i = Some st') ->
      c / st || st'.

         _Proof_: By induction on [i] .

           The hypothesis in the case of 0 will always result in none, giving
           us a contradiction None = Some st' from which anything follows.

           In the case of i = S i', we must consider each possible command in turn

            Skip trivially terminates as in the reduction relation

            Assignment trivially terminates as in the reduction relation

            For sequence of commands, the second command must terminate
            in order for them sequence to, so we must consider the first command.

              If the first command does terminate, then they both terminate as in the reduction
              relation via the induction hypotheses.

              If it does not, we have a contradictory hypothesis of None = Some st'
              from which anything follows.

            For an if statement, which commands must terminate depends on which branch it takes
            so we must consider each case of b.

              Regardless of our choice, they both terminate  as in the reduction relation
              via the induction hypothesis.

            For the while case, its termination depends on the value of its condition b, so we must
            consider each case of b.

              If b is true, then it's sub-command c executes at least once, so we must consider whether it terminates.

                If it does, then so does  each update of state via the induction hypothesis. And the whole loop does by our assumption.

                If it doesn't, then we have a contradiction with our assumption, from which anything follows.

              If b is false, the loop never executes and it trivially terminates as in the reduction relation

          Having shown the hypothesis in each case of a command and each case of induction on the computation depth,
