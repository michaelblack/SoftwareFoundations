...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => andb (evenb x) (gtb x 7)) l.
...
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X := (filter test l, filter (compose negb test) l).
...
Lemma map_snoc : forall (X Y : Type) (f : X -> Y) (x : X) (l : list X), map f (snoc l x) = snoc (map f l) (f x).
  Proof. intros. induction l.
  simpl. reflexivity.
...
Check filter' nat evenb (@cons nat 1 (@cons nat 2 (@cons nat 3 (@nil nat)))).

Check map' nat bool oddb (@cons nat 1 (@cons nat 2 (@cons nat 3 (@nil nat)))).
...
(* We may wish to collect the min and max of a list in whuch case Y would be a pair and X would be a nat *)
...
(* The lhs is saying that whenever you have 3 give true, otherwise always give b. 2 is not 3, so the lhs gives b *)
...
(* Student's note: we could [rewrite <- H. simpl. reflexivity.] in this particular case *)
...
(** Recall the definition of the [index] function:
  ...
  (1) The length of l being n means that l is n constuctors deep. 
  (2) Similarly, index n must dig n past the first constructor or n + 1 constuctors deep.
  (3) By (1) and (2) we see that l does not have enough constuctors for l.
  (4) By (3), index n must encounter nil.
  (5) By (4) and the definition of index, it must return None.
*)
(** [] *)
