...
Proof.
  apply nat_ind.
    simpl. reflexivity.
    intros n IHn.  simpl. apply f_equal. apply IHn.
Qed.
...
Definition plus_one_r'' : forall n:nat, n + 1 = S n :=
  nat_ind (fun n => n + 1 = S n) eq_refl (fun (n : nat) (IHn : n + 1 = S n) => f_equal nat nat S (n + 1) (S n) IHn).
...
(** Write out the induction principle that Coq will generate for the
    ...
    (* forall P : rgb -> Prop, P red -> P green -> P blue -> forall color : rgb, P color *)
...
(** [forall P : natlist1 -> Prop, P nnil1 -> (forall (l : natlist1), P l  -> forall (n : nat), (nsnoc l n)) -> forall l : natlist1, P l ] *)
Check natlist1_ind.
...
Definition plus_assoc''' : nat -> nat -> nat -> Prop :=
  fun n m o => n + (m + o) = (n + m) + o.
  
Definition plus_comm''' : nat -> nat -> Prop :=
  fun n m => n + m = m + n.
  
Check eq_ind.

Check fun n => (eq_ind_r (fun n0 : nat => n0 = n) eq_refl (plus_0_r n)).



Check nat_ind.

Theorem all_plus_comm : forall n m, plus_comm''' n m.
  Proof. 
  intros.
  induction m.
  apply plus_0_r.
  unfold plus_comm'''. rewrite <- plus_n_Sm. simpl. apply f_equal. apply IHm.
Qed.
(** [] *)
