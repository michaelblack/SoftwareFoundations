...
Proof. try (apply le_S). try reflexivity. (* this just does [reflexivity] *) Qed.
...
Proof.
  intros. induction b ; 
  try simpl; try (rewrite optimize_0plus_sound; rewrite optimize_0plus_sound); try reflexivity.
Qed.
...
Definition undistribute_r_simpl (a : aexp) : aexp :=
  match a with
   | APlus (AMult (ANum n1) a1) (AMult (ANum n2) a2) => if beq_nat n1 n2
                                                                                            then AMult (ANum n1) (APlus a1 a2)
                                                                                            else a
  | _ => a
 end.

Fixpoint undistribute_r (a : aexp) : aexp :=
  match a with
    | APlus a1 a2 =>  undistribute_r_simpl (APlus (undistribute_r a1) (undistribute_r a2))
    | AMinus a1 a2 => AMinus (undistribute_r a1) (undistribute_r a2)
    | AMult a1 a2 => AMult (undistribute_r a1) (undistribute_r a2)
    | ANum n => ANum n
  end.

  
Theorem undistribute_r_sound : forall (a : aexp), aeval (undistribute_r a) = aeval a.
Proof. intro a. induction a;
   simpl; try rewrite IHa1; try rewrite IHa2; try reflexivity.
   destruct (undistribute_r a1);
     simpl; try rewrite <- IHa1; try rewrite IHa2; simpl; try reflexivity. 
   destruct a3;
      simpl; try rewrite <- IHa1; try rewrite IHa2; simpl; try reflexivity.
   destruct (undistribute_r a2) ;
      simpl; try rewrite IHa1; try rewrite <- IHa2; simpl; try reflexivity.
   destruct a3; 
    simpl; try rewrite <- IHa1; try rewrite IHa2; simpl; try reflexivity.
  destruct (beq_nat n n0) eqn:e. 
      apply beq_nat_true in e. rewrite e.  simpl. rewrite mult_plus_distr_l. reflexivity.
      simpl. reflexivity.
Qed. 
...
Example presburger_own : forall n m, n < m -> m - n > 0.
...
(** Well see many examples of these in the proofs below. *)
...
(** As is often the case with relations, well find it
    convenient to define infix notation for [aevalR].  Well write [e
    ...
    the evaluation relation is a double down-arrow.  Well typeset it
...
(** In fact, Coq provides a way to use this notation in the definition
    of [aevalR] itself.  This avoids situations where were working on
...
(** Formally, there is nothing very deep about inference rules:
    ...
    defining.  At the moment, our arithmetic expressions dont include
    variables, but well soon be adding them.)  The whole collection
...
(** Note: if youre reading the HTML file, youll see an empty square box instead
of a proof for this theorem.  
You can click on this box to "unfold" the text to see the proof.
Click on the unfolded to text to "fold" it back up to a box. Well be using
...
Theorem aR_aeval : forall (a : aexp), a || aeval a.
  Proof. intro a. apply aeval_iff_aevalR. reflexivity. Qed.
...
Inductive bevalR : bexp -> bool -> Prop :=
  ...
  | E_BAnd : forall e1 e2 b1 b2, e1 \|/ b1 -> e2 \|/ b2 -> BAnd e1 e2 \|/ andb b1 b2
...
Proof.
  split.
    intro H. induction H; try apply aeval_iff_aevalR in H; try apply aeval_iff_aevalR in H0; subst; try reflexivity.
    generalize dependent b. 
      induction e; simpl; intros b H; subst; constructor; try apply aR_aeval; try apply IHe; try apply IHe1; try apply IHe2; try reflexivity.
Qed.
(** [] *)
End AExp.
...
(** For the definitions of evaluation for arithmetic and boolean
    ...
    explicitly if we need them. Functions also take advantage of Coqs
...
(** Lets turn our attention back to defining Imp.  The next thing we
    need to do is to enrich our arithmetic and boolean expressions
    with variables.  To keep things simple, well assume that all
    variables are global and that they only hold numbers. *)

(* ##################################################### *)
(** ** Identifiers *)

(** To begin, well need to formalize _identifiers_ such as program
    variables.  We could use strings for this -- or, in a real
    compiler, fancier structures like pointers into a symbol table.
    But for simplicity lets just use natural numbers as identifiers. *)
...
(** We define a new inductive datatype [Id] so that we wont confuse
...
(** For proofs involving states, well need several simple properties
    of [update]. *)
...
Theorem update_eq : forall n x st,
  (update st x n) x = n.
Proof.
  intros. unfold update. rewrite eq_id. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. unfold update. destruct (eq_id_dec x2 x1 ).
    subst. rewrite eq_id. reflexivity.
    subst. apply neq_id. apply not_eq_sym. apply n.
Qed.
(** [] *)

(** **** Exercise: 3 stars (update_permute)  *)

Theorem neq_refl : forall (X : Type) (x : X) , x <> x -> False.
Proof.
...
Proof.
  intros. unfold update. destruct (eq_id_dec x1 x3), (eq_id_dec x2 x3); 
      subst; try apply neq_refl in H; try contradiction; try reflexivity.
Qed. 
(** [] *)
...
(** (This convention for naming program variables ([X], [Y],
    ...
    types.  Since were not using polymorphism heavily in this part of
...
*)
(**
    For example, heres the factorial function in Imp.
     Z ::= X;;
     Y ::= 1;;
     WHILE not (Z = 0) DO
       Y ::= Y * Z;;
...
(** As usual, we can use a few [Notation] declarations to make things
    ...
    with Coqs built-in notations, so well keep this light -- in
    particular, we wont introduce any notations for [aexps] and
    [bexps] to avoid confusion with the numerical and boolean
    operators weve already defined.  We use the keyword [IFB] for
...
(** Next we need to define what it means to evaluate an Imp command.
    The fact that [WHILE] loops dont necessarily terminate makes defining
    an evaluation function tricky... *)

(* #################################### *)
(** ** Evaluation as a Function (Failed Attempt) *)

(** Heres an attempt at defining an evaluation function for commands,
...
>>
    Coq doesnt accept such a definition ("Error: Cannot guess
    decreasing argument of fix") because the function we want to
    define is not guaranteed to terminate. Indeed, it doesnt always
...
>>
    ...
    would be a disaster for Coqs logical consistency.

    Thus, because it doesnt terminate on all inputs, the full version
...
(** Heres a better way: we define [ceval] as a _relation_ rather than
...
(** This is an important change.  Besides freeing us from the awkward
    ...
    wed want the definition of evaluation to be non-deterministic --
    i.e., not only would it not be total, it would not even be a
    partial function! *)
(** Well use the notation [c / st || st] for our [ceval] relation:
...
Proof. apply E_Seq with (update empty_state X 0). apply E_Ass. simpl. reflexivity.
           apply E_Seq with (update (update empty_state X 0) Y 1). apply E_Ass. simpl. reflexivity.
    ...
    apply E_WhileLoop with (update (update (update (update empty_state X 2) Y 0) Y 2) X 1).
      simpl. reflexivity.
        apply E_Seq with (update (update (update empty_state X 2) Y 0) Y 2);
          apply E_Ass; unfold update; simpl; reflexivity.
...
Theorem XtimesYinZ_spec: forall st st' n m, 
  ...
  XtimesYinZ / st || st' ->
  st' Z = n * m.
Proof. intros st st' n m Xn Ym XY.
  inversion XY. subst. clear XY. unfold update. simpl. reflexivity.
Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
 ...
 | NW_ASS : forall (i : id) (a : aexp), no_whilesR (i ::= a)
 | NW_IF_TRUE : forall (b : bexp) (c1 c2 : com), no_whilesR c1 -> no_whilesR c2 -> no_whilesR (IFB b THEN c1 ELSE c2 FI)
...
Proof.
  split. intros. induction c; try constructor; try simpl in H.
    apply andb_true_elim1 in H. apply IHc1 in H. apply H.
    apply andb_true_elim2 in H. apply IHc2 in H. apply H.
    apply andb_true_elim1 in H. apply IHc1 in H. apply H.
    apply andb_true_elim2 in H. apply IHc2 in H. apply H.
    inversion H.
  intros. induction c; simpl; try reflexivity.
  inversion H.
    apply andb_true_intro. split. apply IHc1. apply H2. apply IHc2. apply H3.
  inversion H.
    apply andb_true_intro. split. apply IHc1. apply H2. apply IHc2. apply H4.
  inversion H.
Qed.
...
Theorem no_whiles_terminating : 
  forall (c : com) , no_whilesR c ->  (forall (st : state), exists st' : state, c / st || st').
  intros c H.
  induction c. intros. exists st. apply E_Skip. 
  intros. exists (update st i (aeval st a)). apply E_Ass. reflexivity.
  inversion H. intro st. apply IHc1 with st in H2.  inversion H2. apply IHc2 with x in H3. inversion H3.
    exists x0. apply E_Seq with x. apply H4. apply H5.
  Focus 2. inversion H.
  inversion H. subst. intros. apply IHc1 with st in H2. apply IHc2 with st in H4.
    destruct (beval st b) eqn:e. 
         inversion H2. exists x. apply E_IfTrue. apply e. apply H0.
         inversion H4. exists x. apply E_IfFalse. apply e. apply H0.
Qed.
...
(** Write a function to evaluate programs in the stack language. It
    ...
    malformed program. *)
Definition s_execute_i (st : state) (stack : list nat) (i : sinstr) : list nat  :=
  match i with
   | SPush n => n :: stack
...
match prog with
  | [] => stack
  | i :: is => s_execute st (s_execute_i st stack i) is
end.
...
Lemma execute_app_seq : forall (st : state) (prog1 prog2 : list sinstr) (stack : list nat),
 s_execute st stack (prog1 ++ prog2) = s_execute st (s_execute st stack prog1) prog2.
  Proof.
  intros st. induction prog1. 
    simpl. reflexivity.
    intros. simpl. rewrite IHprog1. reflexivity.
Qed.


(* Didn't end up needing *)
Lemma always_compiles : forall e, (s_compile e) = [] -> False.
Proof.
  induction e; 
    intros; try simpl in H; try inversion H; try (
    apply (f_equal (A := list sinstr) (B := nat) (length (A := sinstr))) in H; rewrite app_length in H; rewrite app_length in H; simpl in H;  rewrite plus_comm with (m := 1) in H; simpl in H; rewrite  <- plus_n_Sm in H; inversion H).
Qed.

Lemma compile_isolated : forall (st : state) (e : aexp) (stack : list nat),
  s_execute st stack (s_compile e) = aeval st e :: stack.
  Proof. induction e; try (simpl; reflexivity).
    intros. simpl. rewrite execute_app_seq. rewrite IHe1. rewrite execute_app_seq. rewrite IHe2. simpl. rewrite plus_comm. reflexivity.
    intros. simpl. rewrite execute_app_seq. rewrite IHe1. rewrite execute_app_seq. rewrite IHe2. simpl. reflexivity.
    intros. simpl. rewrite execute_app_seq. rewrite IHe1. rewrite execute_app_seq. rewrite IHe2. simpl. rewrite mult_comm. reflexivity.
Qed.

Theorem s_compile_correct : forall (st : state) (e : aexp),
  s_execute st [] (s_compile e) = [ aeval st e ].
Proof.
  intros. apply compile_isolated with (stack := []). Qed.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
 ...
 | E_Ass : forall st x n, CAss x n / st || SContinue /( update st x (aeval st n))
 | E_IF_True : forall b c1 c2 st st' s, beval st b = true -> c1 / st || s / st' -> CIf b c1 c2 / st || s / st' 
 | E_IF_False : forall b c1 c2 st st' s, beval st b = false -> c2 / st || s / st' -> CIf b c1 c2 / st || s / st' 
 | E_Seq_Br : forall c1 c2 st st', c1  / st || SBreak / st' -> CSeq c1 c2 / st || SBreak / st'
 | E_Seq_Cnt : forall c1 c2 st st' st'' s, c1 / st || SContinue / st' -> c2 / st' || s / st'' -> CSeq c1 c2 / st || s / st''
 | E_While_True_Br : forall b c st st', beval st b = true -> c / st || SBreak / st' ->  CWhile b c / st || SContinue / st'
 | E_While_True_Cnt : forall b c st st' st'', beval st b = true -> c / st || SContinue / st' -> CWhile b c / st' || SContinue / st'' -> CWhile b c / st || SContinue / st''
 | E_While_False : forall b c st, beval st b = false -> CWhile b c / st || SContinue / st
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  [ Case_aux c "E_Skip" |
    ...
    Case_aux c "E_Seq_Br" |
    Case_aux c "E_Seq_Cnt" |
    Case_aux c "E_While_True_Br" |
    Case_aux c "E_While_True_Cnt" |
    Case_aux c "E_While_False"
  ].
...
Proof.
 intros b c st st' while b_true. remember (WHILE b DO c END) as loopdef. 
 ceval_cases (induction while) Case; try inversion Heqloopdef; subst. 
  Case "E_While_True_Br".
   exists st. apply while.
  Case "E_While_True_Cnt".
    apply IHwhile2. reflexivity. apply b_true.
  Case "E_While_False".
    rewrite b_true in H. inversion H.
Qed.
...
Theorem ceval_deterministic: forall (c:com) st st1 st2 s1 s2,
     ...
     st1 = st2 /\ s1 = s2.
Proof.
  intros c st st1 st2 s1 s2 H1 H2. generalize dependent s2. generalize dependent st2. ceval_cases (induction H1; intros) Case.
    Case "E_Skip".
    ...
    Case "E_Seq_Br".
      inversion H2. 
        ...
        subst. apply IHceval in H3. apply proj2 in H3. inversion H3.
    Case "E_Seq_Cnt".
       inversion H2. 
        subst. apply IHceval1 in H5. apply proj2 in H5. inversion H5.
        subst. apply IHceval1 in H1. inversion H1. subst. apply IHceval2. apply H6.
   Case "E_While_True_Br".
     inversion H2. 
      subst. split.
        apply IHceval in H8. apply proj1 in H8. apply H8.
        reflexivity.
      subst. apply IHceval in H5. apply proj2 in H5. inversion H5.
      subst. rewrite H7 in H. inversion H.
    Case "E_While_True_Cnt".
      inversion H2.
        subst. apply IHceval1 in H7. apply proj2 in H7. inversion H7.
        subst. apply IHceval1 in H4. apply IHceval2. apply proj1 in H4. rewrite H4. apply H8.
        subst. rewrite H6 in H. inversion H.
    Case "E_While_False".
      inversion H2.
