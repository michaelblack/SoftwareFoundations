...
(* There are an infinite number of ways to show that [8] is [beautiful].
    We can always break the proof into b_0 and the original premise.
    *)
(** [] *)
...
Proof.
   ...
   induction m.
      intros n B. simpl. apply b_0.
      intros n B. simpl. apply b_sum.
                                  apply B.
                                  apply IHm. apply B.
Qed.
...
(** Could this proof also be carried out by induction on [n] instead
    of [E]?  If not, why not? *)
(* No, because proving the property for a specific n tells us nothing 
     about it proving it for the specific n's successor, we are essentially stuck doing the same proof. *)
(** [] *)
...
(** The following proof attempt will not succeed.
  ...
  This fails because there is no constructor that results in ev (S n) from n.
...
(* The same thing, only we don't add the equalities to n based on constructors to the context. *)
(** [] *)
...
Proof.
  intros n ev4.
  inversion ev4 as [| n' ev2]. 
  inversion ev2 as [| n'' evn].
  apply evn.
Qed.
...
Proof.
  ...
  induction en.
      ...
      simpl in enm. inversion enm as [| n' enm']. apply IHen in enm'. apply enm'.
Qed.
(** [] *)
...
Inductive pal {X : Type} : list X -> Prop :=
  ...
  | pal_cons : forall (x : X) (l : list X), pal l -> pal (cons x (snoc l x)).
...
Proof.
  ...
  induction p.
    reflexivity.
    reflexivity.
    simpl. rewrite rev_snoc. simpl. rewrite <- IHp. reflexivity.
Qed.
...
Theorem lte_S : forall (n  m : nat), S n <= m -> n <= m.
Proof.
  intros.
  apply le_S in H.
...
Proof.
  ...
  apply le_ble_nat.
  apply ble_nat_true in n_m.
  apply ble_nat_true in m_o.
...
R 2 2 6 is not provable. To decrease π_3 you must descrease π_1 or π_2.

No, any the constructors are either symmetrical in n and m or have a mirror (e.g c2 and c3).

No, in order to contruct the premise of c4 is from c2 and c3 and c4 undoes that work bringing you no further.
*)
...
Theorem R_add : forall n m o, R n m o -> n + m = o.
Proof.
  intros.
  induction H.
  simpl. reflexivity.
...
Theorem add_R : forall n m o, n + m = o -> R n m o.
Proof.
  intros.
  ...
  intros.
      ...
      inversion H.
      apply c3. apply IHo. simpl in H. inversion H. simpl. reflexivity.
      apply c2. apply IHo. simpl in H. inversion H. reflexivity.
Qed.

Theorem R_is_Addition : forall  n m o, R n m o <-> n + m = o.
  Proof. split. apply R_add. apply add_R. Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
 | sub_nil : forall ys, subseq [] ys
 | sub_cons : forall n xs ys , subseq xs ys -> subseq (n :: xs) (n :: ys)
 | nosub_cons : forall n xs ys, subseq xs ys -> subseq xs (n :: ys).
 
Theorem subeq_refl : forall xs, subseq xs xs.
Proof.
intros.
induction xs.
apply sub_nil.
apply sub_cons. apply IHxs.
Qed.

Theorem subseq_app : forall xs ys zs, subseq xs ys -> subseq xs (ys ++ zs).
Proof.
  intros.
  induction H.
    apply sub_nil.
    simpl. apply sub_cons. apply IHsubseq.
    simpl. apply nosub_cons. apply IHsubseq.
Qed.

Theorem subseq_trans : forall xs ys zs, subseq xs ys -> subseq ys zs -> subseq xs zs.
  intros xs ys zs x_y. generalize dependent zs.
    induction x_y.
    intros. apply sub_nil.
    intros. induction zs. inversion H.
    inversion H. apply sub_cons. apply IHx_y. apply H1.
    apply nosub_cons. apply IHzs. apply H2.
    intros.  induction zs. inversion H. inversion H. apply nosub_cons. apply IHx_y. apply H1. apply nosub_cons. apply IHzs. apply H2.
Qed. 
...
intros Podd Peven n odd even.
destruct (oddb n).
    apply odd. simpl. reflexivity.
    apply even. simpl. reflexivity.
Qed.
...
Proof.
  unfold combine_odd_even.
  intros Podd Peven n combine.
  destruct (oddb n). 
      intros. apply combine.
      intros. inversion H.
Qed.
...
Proof.
  unfold combine_odd_even.
  intros Podd Peven n combine even.
  destruct (oddb n).
      inversion even.
      apply combine.
Qed.
